...
class TransactionPipeline:
    ...
    def fallback_classify(self, merchant_raw, merchant_clean, description=""):
        """
        Enhanced fallback with MERCHANT-FIRST priority rules
        Fixes: MCDONALD'S, TARGET, SHELL, UBER EATS misclassifications
        """
        text = f"{merchant_raw} {merchant_clean} {description}".lower()
        
        # STEP 1: MERCHANT-FIRST RULES (Highest Priority)
        # These override everything - specific merchant names
        
        merchant_first_rules = {
            'mcdonald': 'food and dining',
            'netflix.com': 'entertainment',
            'spotify.com': 'entertainment',
            'wal-mart': 'shopping',
            'amzn': 'shopping',
            '7-eleven': 'food and dining',
            '7 eleven': 'food and dining',
            'mcdonalds': 'food and dining',
            'starbucks': 'food and dining',
            'burger king': 'food and dining',
            'wendys': 'food and dining',
            'taco bell': 'food and dining',
            'chipotle': 'food and dining',
            'uber eats': 'food and dining',  # "eats" takes priority over "uber"
            'doordash': 'food and dining',
            'grubhub': 'food and dining',
            ...
            'target': 'shopping',  # Fix: TARGET → shopping (not fees)
            'walmart': 'shopping',
            'amazon': 'shopping',
            'costco': 'shopping',
            'best buy': 'shopping',
            ...
            #'zoom': 'subscriptions',
            'mta': 'transportation',
            'uber': 'transportation',  # "uber" alone = transportation
            'lyft': 'transportation',
            'taxi': 'transportation',
            'hilton': 'travel',
            'marriott': 'travel',
        ...
        # Check merchant-first rules
        for merchant_key, category in merchant_first_rules.items():
            if merchant_key in text:
                return category, 0.75, f"merchant_first:{merchant_key}"
        
        # STEP 2: KEYWORD RULES (Lower Priority)
        # Only run if no merchant match
        
        keyword_rules = {
            'transportation': {
                'keywords': ['gas', 'fuel', 'parking', 'toll', 'transit', 'subway', 'bus', 'train'],
                'weight': 1
            },
            'healthcare': {
                'keywords': ['pharmacy', 'prescription', 'doctor', 'medical', 'hospital', 'clinic', 'dental', 'vision'],
                'weight': 1
            },
            'food and dining': {
                'keywords': ['restaurant', 'cafe', 'coffee', 'pizza', 'sushi', 'burger', 'food'],
                'weight': 1
            },
            'entertainment': {
                'keywords': ['movie', 'theater', 'cinema', 'concert', 'show', 'streaming'],
                'weight': 1
            },
            'utilities': {
                'keywords': ['electric', 'water bill', 'gas bill', 'internet', 'phone bill', 'mobile'],
                'weight': 1
            },
            'shopping': {
                'keywords': ['store', 'retail', 'mall', 'outlet', 'boutique'],
                'weight': 1
            },
            'groceries': {
                'keywords': ['grocery', 'supermarket', 'produce', 'organic'],
                'weight': 1
            },
            'income': {
                'keywords': ['payroll', 'salary', 'deposit', 'paycheck', 'refund', 'reimbursement'],
                'weight': 1
            },
            'travel': {
                'keywords': ['hotel', 'motel', 'flight', 'airline', 'vacation', 'resort'],
                'weight': 1
            },
            'subscriptions': {
                'keywords': ['subscription', 'membership', 'saas', 'software license'],
                'weight': 1
            },
            'fees': {
                'keywords': ['late fee', 'overdraft fee', 'service fee', 'atm fee', 'monthly fee', 'penalty'],
                'weight': 1
            }
        }
    ...
    def categorize(self, merchant, description, amount):
        ...
        Fallback only runs when confidence < 0.5
        """
        ...
        else:  # < 0.5 → try fallback
            fallback_cat, fallback_conf, fallback_rule = self.fallback_classify(merchant, merchant, description
            )
